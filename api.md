...
闪动原因：使用web-components之后，所有的自定义标签的代码都是通过js代码定义的，因此要先加载js文件，相应的自定义标签才能生效。通常为了使页面加载速度更快，大家都会习惯在页面最后引入`script`外链，因此在js文件加载完成之前，浏览器并不知道这些自定义标签到底应该如何渲染，所以中间会有一个短暂的时间空挡期。

解决原理： `ox-cloak`属性其实是设置了css属性为`opacity:0` ，这样使得整个页面在JS代码加载好之前保持看不见的状态，等JS加载完成之后，通过js代码移除这个属性。
